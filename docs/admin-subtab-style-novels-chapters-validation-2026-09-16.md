...
### 3G.6 PO18.tw 账号面板布局重做

**触发**：用户在登录态截图中指出该面板「前段留白异常」并要求重新设计布局。经核对，异常不是 padding 漏配，而是**布局模型本身选错**。

**旧布局的问题（实测）**：左右两栏并排（`lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]`），左栏是账号表单、右栏是 Cookie 兜底。左栏内容天然少于右栏，被网格等高拉伸后，按钮下方空出 **151px** 死留白——正是截图所见的「前段留白」。此外验证码挑战区被放在整个面板最底部，与触发它的「获取验证码」按钮隔了一屏；「清除账号」这个破坏性操作与常规操作同权重并排。

**设计判断**：这两栏不是并列信息，而是**主路径与备路径**——账号登录是主，Cookie 兜底是它失败时的替代。并排会让人误读为二选一。故改为纵向分段：主路径在上，备路径用次级表面（`--admin-panel-muted` + 描边）表明从属；验证码紧跟登录动作；破坏性操作隔离到页脚并与说明文字并置。

**实测（真实浏览器，6 档视口）**：

| 视口 | 账号字段列 | 兜底字段列 | 验证码位置 | 横向溢出 |
| --- | --- | --- | --- | --- |
| 1440 | 2 | 2 | 主路径段内 | 无 |
| 1024 | 2 | 2 | 主路径段内 | 无 |
| 901 | 2 | 2 | 主路径段内 | 无 |
...
断点精确落在 901/900（与全局规范一致），三段间距统一 24px，页面 `scrollWidth === innerWidth`。对比度实测：亮色下段落标题 16.60:1、辅助文字 5.12:1；暗色下 10.77:1 / 5.34:1，全部 PASS。焦点环 `outline: solid 2px` 正常。控制台无错误。

**自曝的两处返工**：第一轮实现后截图发现我**自己又制造了同类缺陷**——兜底区把「加密保存 Cookie」按钮放在左列字段下方，导致右列比左列矮、底部困住空白。修正为按钮移出字段网格、独立成行。同时手机端验证码图片独占一行，补 `@media (max-width: 560px)` 让图片与输入框同排、按钮独占一行。这两处都不是规范要求的，是第一轮自测不充分留下的，已一并修正。

**顺带清理**：改造后 `components/admin/AdminPanel.tsx`（`Card` 薄包装）失去唯一消费者，成为零引用死代码，已删除；手册 §5 中「唯一消费者是 `Po18AccountPanel.tsx:166`」的记录同步更正。

**接口与逻辑零改动**：逐字比对确认所有 `onClick`、`disabled` 条件、`busy` 状态分支、`scrapeApi` 调用与 `confirm` 流程均未变化，仅位置移动。
