...
**规则：卡片类面板（`Card` + `AdminPanelHeading`）标题一律带图标；表格类面板（`AdminDataPanel`）标题一律不带图标。**

这条规则在 2026-09-16 全站审计后确立，此前并未成文，但代码里已接近自洽（`AdminDataPanel` 26/26 不带、`Card` 26/28 带），仅 2 处例外，已补齐。

图标写法固定为 `<span className="admin-panel-title"><Icon className="size-4 text-primary" aria-hidden="true" />标题</span>`，尺寸 16px、颜色 `--color-primary`。`.admin-panel-title`（`:93`）提供 `inline-flex + gap: 0.5rem`，不要各页重写 flex/gap。

为什么不给表格面板加图标：表格标题区比卡片标题区紧凑，且表格面板已有列头、工具栏承担定位；再加图标会与密集的行信息争注意力。反过来，卡片之间需要快速区分，图标是低成本手段。

选择图标时的注意事项：**同一图标不要跨语义无关的面板复用**。审计发现 `Route` 一个图标被用在「近 7 日访问趋势」「访问来源」「更新趋势」「更新活跃度」「本周运营关注」5 处，`BarChart3` 与 `ShieldAlert` 各 3 处——17 种图标里 4 种被复用，覆盖 26 个带图标面板中的 13 个（50%）。复用到这个程度后图标不再携带信息，退化成装饰。新增面板时优先选与面板语义直接对应的图标；确实找不到合适图标时，宁可换用同语义族内的另一个，也不要复用已承载其他含义的图标。

#### 4.5.2 卡片内标题层级

卡片内部的子分区标题（如 PO18 面板的「账号登录」「浏览器 Cookie 兜底」）需与上下两级都可区分：

| 层级 | 字号/字重 | 图标 |
| --- | --- | --- |
| 面板标题（`AdminPanelHeading` h3） | 20px / 750 | 16px 强调色 |
| 子分区标题 | 15px / 600 | 16px 强调色 |
| 字段标签（`Label`） | 14px / 500 | 无 |

此前子分区标题是 14px/700，与字段标签的 14px 只有 200 字重之差，字号完全相同，在页面上几乎看不出层级；其图标又是 14px 灰色，与面板标题的 16px 强调色图标各说各话。**子分区标题与字段标签必须有字号差（≥1px），不能只靠字重区分。**

子分区标题**不加顶部分割线**：面板标题所在的 `.admin-panel-heading` 已有 `border-bottom`（`:7090`），首个分区再画线是重复；带自身边框的分区（如 `--fallback`）其分割线会与容器边框在半行内形成双线（实测相距 17px、同色）。分隔由现有元素承担。
